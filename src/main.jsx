...
const DEFAULT_DATA = {
  ...
  hero: { kicker: "DIGITAL MARKETING & WEB DEVELOPMENT AGENCY", titleA: "Digital Solutions", titleB: "That Drive", titleC: "Growth.", description: "We design and develop modern websites, Shopify stores, and digital experiences that help businesses build a stronger online presence.", images: ["", "", ""] },
...
function ProjectCard({ project, index }) {
  ...
  // Priority: admin-uploaded custom image first (never overridden), then live screenshot, then legacy thum.io, then a guaranteed placeholder.
  const candidates = [customImage, microlinkUrl, thumIoUrl, generatedThumb].filter(Boolean);
        ...
        {tab==="projects" && <div className="admin-projects">
          ...
          {draft.projects.map((p,i)=><div className="admin-project" key={p.id}>
            ...
            <div className="thumbnail-upload">
              ...
              <a className="thumbnail-preview" href={p.image && !isAutoGeneratedUrl(p.image) ? p.image : (hasWebUrl(p.url) ? microlinkThumb(p.url.trim()) : makeThumb(p, i))} target="_blank" rel="noreferrer"><img src={p.image && !isAutoGeneratedUrl(p.image) ? p.image : (hasWebUrl(p.url) ? microlinkThumb(p.url.trim()) : makeThumb(p, i))} alt="Automatic or custom thumbnail" onError={e=>{e.currentTarget.src=makeThumb(p,i);}} /> <span>{p.image && !isAutoGeneratedUrl(p.image) ? "Custom image \u2014 will always be used" : "Automatic live preview (upload to override)"}</span></a>
            </div>
